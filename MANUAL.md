...
### Today page
- **Three sections, in order:** TODAY → SOMEDAY → TOMORROW.
- The **Pending** number at the top counts *today + someday only*.
- **Tomorrow** is read-only: open *Timeline* to edit those tasks.
- Clicking a checkbox no longer reloads the page — the row fades out.

### New-task defaults
When you open *+ New task*, the form pre-fills:
- **Due date** = right now + 2 hours, rounded up to the next half-hour.
- **Reminder** = 5 minutes before.

Defaults are **only** applied when creating; editing leaves your saved
values alone.

### Natural-language dates
Type a phrase in the **title** and the due-date fills in:
- `tomorrow`, `today`
...
A small "Parsed:" hint appears under the title with a *clear* link.

### Someday tasks
Leave the date field blank when creating a task and it lands in the
**SOMEDAY** bucket on the dashboard. Set a date later to schedule it.

### Urgent priority
A new priority above *High*. Urgent tasks:
- show a red pill with a white label,
- get a red left border on every list,
- sort to the top within their group.

### Categories
Settings → *Category names* lets you rename Work / Study / Personal /
Health. Colours are fixed (blue / purple / green / teal). The dots on
the calendar use the highest-precedence category for that day.

### Calendar
- Each day with at least one pending task gets a **single dot** coloured
  by category (Urgent wins over everything).
- Click a day to see the tasks (and any public holidays) for it.
- The list view rows now read *"Monday, Apr 28"*.
- Settings → *Show Zambian public holidays* adds 13 standard holidays as
  gray, non-editable events.

### Timeline
- Groups read PAST → TODAY → TOMORROW → FUTURE, always in that order.
- The list scroll is capped (500 px desktop, 400 px mobile) so the
  *+ New task* button stays reachable.
- A floating **📅 Today** pill appears in the bottom-right when you
  scroll away from the *Today* group.

### Pomodoro
- The *Reset* button now asks *"Reset timer? Your progress will be
  lost."* before clearing.
- When the timer ends you get **two short beeps** and a **browser
  notification**. Each is independently togglable in Settings.

### Activity log
- New page in the sidebar (and mobile nav). Shows the last 30 actions
  you've taken: created / updated / completed / re-opened / deleted /
  restored. Deleted tasks link straight to the Trash for one-click
  restore.

### Demo data
A management command seeds a realistic spread:

```bash
venv/bin/python manage.py seed_demo --user=milimo
```

Re-running cleans the previous demo tasks first.
